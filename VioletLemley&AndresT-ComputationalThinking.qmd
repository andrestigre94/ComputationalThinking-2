...
author: "Violet Lemley & Andres Tigreros"
format: pdf
editor: visual
---
...
ANSWER: The output is "11 is greater than 10"
...
ANSWER: RESEARCHERS MEASURED STRESS BY TAKING SAMPLES OF GCM (Glucocorticoid Metabolite) per gram
...
ANSWER: A row in this datasets represent each measurement of GCM/gram for a determined individual wit its coordinates in UTM, sex, elevation and location
...
Hopefully we can now visualize what that `case_when()` statement did with the information we gave it.
...
for (i in 1:length(data$animal_weight)) {
  print(data$animal_weight[i])
  
  if(is.na(data$animal_weight[i])){
  } else{
    
    if (data$animal_weight[i] <= 500) {
      
      print("Starving")
    } else {
      
      if (data$animal_weight[i] >= 500 &data$animal_weight[i] <= 1500) {
...
ANSWER: We are going to create a new column for the dataset in which we will add the same values we ran in the loop
...
data1 <- data %>% 
  mutate(weight_cat = case_when(
    animal_weight < 500 ~ "starving",
    animal_weight >= 500 & animal_weight <= 1500 ~ "Doing good",
    animal_weight >1500 ~ "need cardio",
    .default = "NA"
  ))
